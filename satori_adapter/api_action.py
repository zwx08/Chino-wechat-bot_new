...
class ApiReturn_wxid_details_data(BaseModel):
    ...
    source: int
    bigHeadImgUrl: HttpUrl
    smallHeadImgUrl: HttpUrl
...
class update_guild_member_details(base_api_action):  # TODO:
    """更新群成员资料(2113)
...
class GetChatroomList_data_RecordInfo(BaseModel):
    chatRoomName: str
    nickName: str
    remark: str
    chatRoomFlag: int
...
class GetChatroomList_data(BaseModel):
    record: List[GetChatroomList_data_RecordInfo]
    total: int
    count: int
    page: int
    pageSize: int
